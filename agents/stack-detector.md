...
name: stack-detector
description: >
  Inspect a repository and produce a machine-usable stack profile: languages,
  frameworks, test runners and commands, lint/format/build commands, layers, and
  which project expert-agents fit. Use during sdd-init, or when config.yaml is
  missing or stale.
model: inherit
