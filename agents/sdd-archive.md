...
name: sdd-archive
description: >
  Archive a completed and verified change: merge delta specs into main specs, move the
  change folder to the archive, persist the final report. Use after verification
  passes. Completes the SDD cycle.
model: inherit
...
- In `next_recommended`, always suggest running `sdd-steer` to refresh the project
  steering docs after the cycle closes.
