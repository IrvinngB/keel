...
name: sdd-security
description: >
  Language-agnostic security review (OWASP-style checklist) of a change's diff or an
  explicit scope of files. Use standalone or from sdd-verify when security_review is
  on. Complements stack experts; covers what stack reviews tend to skip.
model: inherit
...
| Dependencies | Newly added deps (check name/purpose); known-dangerous patterns in usage |
