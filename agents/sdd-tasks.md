...
name: sdd-tasks
description: >
  Break a change into an implementation task checklist with a review workload
  forecast. Use when spec and design are ready and the change must be sliced into
  ordered, actionable work items before apply.
model: inherit
...
1. Read `proposal.md`, `specs/`, `clarifications.md` and `design.md` (required).
2. Extract every file to create/modify/delete from the design; determine dependency
   order and test requirements.
