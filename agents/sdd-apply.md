...
name: sdd-apply
description: >
  Implement code changes from the task checklist. Use when tasks are ready and code
  must be written following specs and design. Reads tasks/spec/design, implements,
  tests, and marks tasks complete.
model: inherit
...
5. For each assigned task: read its spec scenarios (acceptance criteria), the
   design constraints, existing code patterns → write the code → mark `- [x]` in
   `tasks.md` immediately → note deviations.
