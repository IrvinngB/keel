## SDD — Spec-Driven Development ({displayName})

This project uses SDD (Spec-Driven Development). The pipeline contract lives in
the `{{skill:sdd-workflow}}` skill: read it before any SDD work and act as the orchestrator it
defines (route phases, own `<change>/state`, ask the user exactly one question at a
time).

Entry points: {{cmd:init}} once per project, {{cmd:new}} to start a change,
{{cmd:continue}} for the next dependency-ready phase.
