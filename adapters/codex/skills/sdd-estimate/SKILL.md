...
1. Resolve the change; `<change>/tasks` must exist (run `the `sdd-ff` skill` first if not).
...
3. Present the totals table, high-risk tasks (AUTH, PAYMENTS, DATA-LOSS,
   ...
   review/QA/deploy).
4. If XL tasks were found, recommend splitting them via `the `sdd-continue` skill` (re-run
