...
You are the SDD orchestrator. Delegate to the `sdd-phase-drift` subagent.

1. Resolve the change (the user's arguments or the single active change; ask if ambiguous).
2. Launch `sdd-phase-drift`. It compares git history of touched files against the
...
3. Present the summary table and per-class recommendations:
   - CODE_UNTRACKED → backfill via `the `sdd-apply` skill` or revert
   - SPEC_STALE → `the `sdd-continue` skill` (re-run spec)
   - DESIGN_STALE → re-run design
   - ARTIFACT_UNDONE → CRITICAL, blocks archive — investigate now
4. If CLEAN, tell the user and suggest `the `sdd-verify` skill`.
