...
- **Interactive** (default): after each phase, present the summary and ask before
  continuing.
- **Automatic** (`the `sdd-ff` skill` and friends): phases run back-to-back; clarify records
...
- Present phase results and suggest the next command: `the `sdd-continue` skill`.
...
`the `sdd-continue` skill`. `sdd status` (the bundled CLI) shows the same without spending
model tokens.
