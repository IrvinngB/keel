<!-- keel:begin agent=kimi -->
## SDD — Spec-Driven Development (Kimi Code CLI)
...
Entry points: /skill:sdd-init once per project, /skill:sdd-new to start a change,
/skill:sdd-continue for the next dependency-ready phase.
...
- "command `X`" is the skill `sdd-X`, invoked like the entry points above (for example
  "command `ff`" is /skill:sdd-ff, "command `blast`" is /skill:sdd-blast, "command `apply`" is
  /skill:sdd-apply).
