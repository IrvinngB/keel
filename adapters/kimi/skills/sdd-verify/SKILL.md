...
1. Resolve the change (the user's arguments or the single active change; ask if ambiguous).
   `<change>/apply-progress` must exist — if not, run `/skill:sdd-apply` first.
2. Optional pre-check: if the change has been open a while or commits landed
   outside apply, suggest `/skill:sdd-drift` first.
...
Next: `/skill:sdd-archive` if clean, `/skill:sdd-apply` again if FAIL. If
`security_review: true` in `config` (or the user asks), also run `/skill:sdd-security`.
