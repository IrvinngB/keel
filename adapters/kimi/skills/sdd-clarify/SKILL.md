...
1. Resolve the change; `<change>/proposal` and `<change>/spec/*` must exist (run `/skill:sdd-ff` steps
   first if not).
...
5. Update `<change>/state` (`current_phase: clarify`). Next: `/skill:sdd-continue` → design.
