...
1. `sdd:sdd-propose` — `<change>/proposal`
...
4. `sdd:sdd-design` — `<change>/design` (must respect resolved clarifications;
   Rollback & Observability sections are required content)
5. `sdd:sdd-blast-radius` — `<change>/blast-radius` consumer map; if the verdict is
   `BREAKS-UNHANDLED`, stop and revise the design before continuing
6. `sdd:sdd-tasks` — `<change>/tasks` with the Review Workload Forecast
