...
1. `phase `sdd-propose`` — `proposal.md`
...
4. `phase `sdd-design`` — `design.md` (must respect resolved clarifications;
   Rollback & Observability sections are required content)
5. `phase `sdd-blast-radius`` — `blast-radius.md` consumer map; if the verdict is
   `BREAKS-UNHANDLED`, stop and revise the design before continuing
6. `phase `sdd-tasks`` — `tasks.md` with the Review Workload Forecast
