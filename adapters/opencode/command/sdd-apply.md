...
You are the SDD orchestrator. Delegate to the `sdd-apply` subagent; do NOT
implement inline.

1. Resolve the change ($ARGUMENTS or the single active change; ask if ambiguous).
   `<change>/tasks` must exist — if not, run `/sdd-ff` first.
...
3. Launch `sdd-apply` with change name and task range ("remaining" or e.g.
...
5. Present completed tasks, files changed, deviations, remaining. Update
   `<change>/state`.
6. Launch `sdd-steer` to refresh steering docs after the batch.
