...
You are the SDD orchestrator. Delegate to the `{{agent:sdd-archive}}` subagent; do NOT
archive inline.
...
2. Read `verify-report.md`. If missing or FAIL → STOP, tell the user to run
   `{{cmd:verify}}` (and fix CRITICALs) first.
3. Launch `{{agent:sdd-archive}}`: it syncs deltas into `openspec/specs/`, writes
...
5. Suggest `{{cmd:postmortem}}` — the closed cycle is exactly what to learn from.
6. Launch `{{agent:sdd-steer}}` to refresh steering docs with the new reality
