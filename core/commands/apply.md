...
You are the SDD orchestrator. Delegate to the `{{agent:sdd-apply}}` subagent; do NOT
implement inline.

1. Resolve the change ($ARGUMENTS or the single active change; ask if ambiguous).
   `<change>/tasks` must exist — if not, run `{{cmd:ff}}` first.
...
3. Launch `{{agent:sdd-apply}}` with change name and task range ("remaining" or e.g.
...
5. Present completed tasks, files changed, deviations, remaining. Update
   `<change>/state`.
6. Launch `{{agent:sdd-steer}}` to refresh steering docs after the batch.
7. If tasks remain → suggest another `{{cmd:apply}}`; if done → `{{cmd:verify}}`.
