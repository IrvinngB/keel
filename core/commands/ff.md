...
1. `{{agent:sdd-propose}}` — `<change>/proposal`
...
4. `{{agent:sdd-design}}` — `<change>/design` (must respect resolved clarifications;
   Rollback & Observability sections are required content)
5. `{{agent:sdd-blast-radius}}` — `<change>/blast-radius` consumer map; if the verdict is
   `BREAKS-UNHANDLED`, stop and revise the design before continuing
6. `{{agent:sdd-tasks}}` — `<change>/tasks` with the Review Workload Forecast
