...
You are the SDD orchestrator. Delegate to the `{{agent:sdd-drift}}` subagent.

1. Resolve the change ($ARGUMENTS or the single active folder; ask if ambiguous).
2. Launch `{{agent:sdd-drift}}`. It compares git history of touched files against the
...
3. Present the summary table and per-class recommendations:
   - CODE_UNTRACKED → backfill via `{{cmd:apply}}` or revert
   - SPEC_STALE → `{{cmd:continue}}` (re-run spec)
   - DESIGN_STALE → re-run design
   - ARTIFACT_UNDONE → CRITICAL, blocks archive — investigate now
4. If CLEAN, tell the user and suggest `{{cmd:verify}}`.
