...
- **{{agent:sdd-blast-radius}}** maps every real consumer of the code about to change (call
  sites, routes, tables, published APIs, tests) AFTER design; tasks must convert
  every must-handle line into work items.
- Auxiliary (run anytime): `{{agent:stack-detector}}` (inside init),
  `{{agent:sdd-drift}}` (before verify/archive), `{{agent:sdd-security}}` (inside verify or
  standalone), `{{agent:sdd-estimate}}` (after tasks), `{{agent:sdd-steer}}` (after apply
...
- If a decision is needed and unresolved, the orchestrator asks BEFORE apply:
  ...
  uses a colon).
- `{{agent:sdd-apply}}` must not start oversized work without a resolved decision; when
...
- **Interactive** (default): after each phase, present the summary and ask before
  continuing.
- **Automatic** (`{{cmd:ff}}` and friends): phases run back-to-back; clarify records
